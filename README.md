...
* [consul-images](https://hub.docker.com/_/consul) 镜像版本已经换成和开发环境一致的1.5.1
...
```
git clone git@192.168.5.29:ops/consul_cluster_on_aws_eks.git
```

进入到 `consul_cluster_on_aws_eks` 目录:

```
cd consul_cluster_on_aws_eks
```
...
```
如果命令不存在，跑以下脚本下载以下相关命令
```
ssl-command-download.sh
#!/bin/bash
...
### 生成 Consul Gossip 加密密钥 （如果已有直接将 GOSSIP_ENCRYPTION_KEY变量设置成对应值即可）
...
### 使用以下命令生成Consul 集群使用的Secret 和 Configmap （证书之前已经有的话，自己替换成功相关证书即可，如果名字有变化记得修改configs/*里面部署文件证书的路径名称）
...
kubectl create configmap consul-cli --from-file=configs/server-cli.json
```
### 在创建Consul service和statefulSet之前，先创建Consul集群使用到的pv(persistVolume)持久化存储卷和pvc(persistVolumeClaim)持久化存储卷声明 (根据集群存储情况灵活选择，例如aws不需要此步，直接在statefulset文件通过存储卷模板分配
```
...
3. 可以通过ingress将web界面暴露出去，然后在nginx-ingress上面配置账号密码认证，添加以下配置
具体参考 https://kubernetes.github.io/ingress-nginx/examples/auth/basic/

apiVersion: extensions/v1beta1
kind: Ingress
metadata:
  annotations:
    nginx.ingress.kubernetes.io/auth-realm: Authentication Required - leedarson
    nginx.ingress.kubernetes.io/auth-secret: leedarson
